...
export default function AllClasses() {
  ...
  return (
    <section className="px-30 py-20 bg-[#e9f0e4]/40 backdrop-blur-xl border border-[#c7d6b8]/40 shadow-lg">
      <div className="flex flex-col items-center gap-6 mb-12 border-b border-[#c7d6b8]/30 pb-8">
        <div className="text-center">
          <h2 className="text-4xl font-bold text-[#2F3A2F]">All Classes</h2>
          <p className="mt-2 text-[#5D6B57]">
            Join expertly designed training sessions
          </p>
        </div>
        ...
        <div className="relative w-full lg:max-w-xl">
          ...
          <input
            ...
            value={searchQuery}
            onChange={handleSearchChange}
            className="w-full pl-12 pr-16 py-3 rounded-full border border-[#A3B18A] bg-white/80 text-[#2F3A2F] focus:outline-none focus:ring-2 focus:ring-[#2F3A2F]/20 placeholder-[#5D6B57]/60 transition-all text-sm"
          />
          ...
          <button
            ...
            className={`absolute inset-y-0 right-2 my-auto h-9 w-9 rounded-full flex items-center justify-center transition ${
              selectedCategory !== "All"
                ? "bg-transparent text-[#6B8E23]"
                : "bg-transparent text-[#2F3A2F]"
            }`}
          >
          ...
          {showCategories && (
            <div className="absolute right-0 top-14 z-50 w-64 rounded-3xl border border-[#A3B18A]/40 bg-white/90 p-3 shadow-xl backdrop-blur-xl">
              <p className="px-3 pb-2 text-xs font-semibold uppercase tracking-wide text-[#5D6B57]">
                Filter by category
              </p>
              ...
              <div className="flex flex-col gap-1">
                {categories.map((category) => (
                  <button
                    ...
                    className={`rounded-2xl px-4 py-2 text-left text-sm font-medium transition ${
                      selectedCategory === category
                        ? "bg-[#6B8E23] text-white"
                        : "text-[#2F3A2F] hover:bg-[#e9f0e4]"
                    }`}
                  >
                    {category}
                  </button>
        ...
        {selectedCategory !== "All" && (
          <div className="flex items-center gap-3 rounded-full bg-white/70 px-4 py-2 text-sm text-[#2F3A2F]">
            <span>
              Category: <strong>{selectedCategory}</strong>
            </span>
            <button
              onClick={() => handleCategorySelect("All")}
              className="font-semibold text-[#6B8E23]"
            >
              Clear
            </button>
          </div>
      ...
      {loading ? (
        ...
        <div className="h-96 flex flex-col justify-center items-center text-center">
          <p className="text-xl font-medium text-[#2F3A2F]">No classes found</p>
          <p className="text-sm text-[#5D6B57] mt-1">
