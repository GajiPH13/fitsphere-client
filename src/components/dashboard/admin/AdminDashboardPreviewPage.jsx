...
//           <p className="text-lg font-semibold text-[#2F3A2F] animate-pulse">
...
//             strokeLinecap="round"
//             strokeLinejoin="round"
//             d="M12 6v12m-3-2.818l.214.116M12 6.5l.214-.116M6 12h12m-9.43 5.93A3.75 3.75 0 016 14.25M17.13 6.09A3.75 3.75 0 0118 9.75"
//           />
//         </svg>
//       ),
//     },
...
//               Track metrics, review classes, evaluate financials, and approve
//               pending trainer verification flows.
//             </p>
//           </div>
...
//                   tickLine={false}
//                   axisLine={false}
//                   tick={{ fill: "#5D6B57", fontSize: 12, fontWeight: 600 }}
//                 />
//                 <YAxis
//                   allowDecimals={false}
//                   tickLine={false}
//                   axisLine={false}
//                   tick={{ fill: "#5D6B57", fontSize: 12 }}
//                 />
//                 <Tooltip
...
//                   {stats?.pendingTrainerApplications || 0}
//                 </h3>
//                 <span className="text-xs font-bold text-[#6B8E23] bg-white/60 px-2 py-1 rounded-md">
//                   Action Required
//                 </span>
//               </div>
//             </div>
...
//   return (
//     <div className="rounded-[32px] border border-white/60 bg-white/40 p-6 shadow-xl backdrop-blur-xl flex flex-col justify-between">
//       <h2 className="mb-5 text-xl font-black tracking-tight text-[#2F3A2F]">
//         {title}
//       </h2>
...
//             {item.name}
//           </span>
//           <span className="mt-0.5 text-base font-black text-[#2F3A2F]">
//             {item.value}
//           </span>
//         </div>
//       ))}
...
//         <p className="text-xs font-bold text-[#5D6B57] uppercase tracking-wider">
//           {payload[0].name}
//         </p>
...
export default function AdminDashboardPage() {
  ...
  if (isPending || loading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#EDF3E7]">
        <div className="rounded-xl border border-white/40 bg-white/40 px-6 py-4 shadow-xl backdrop-blur-md">
          <p className="text-sm font-semibold text-[#2F3A2F] animate-pulse">Loading admin terminal...</p>
        </div>
      </div>
    );
  }
  ...
  if (role !== "admin") {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#EDF3E7] px-4">
        <div className="max-w-sm rounded-2xl border border-white/50 bg-white/50 p-6 text-center shadow-xl backdrop-blur-xl">
          <h1 className="text-2xl font-black text-[#2F3A2F]">Access Denied</h1>
          <p className="mt-2 text-xs text-[#5D6B57]">Only approved administrators can access this node.</p>
        </div>
      </main>
    );
  }
  ...
  if (error) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#EDF3E7]">
        <div className="rounded-xl border border-red-200/40 bg-red-50/50 px-6 py-4 shadow-xl backdrop-blur-md text-xs text-red-600 font-bold">{error}</div>
      </div>
    );
  }
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-[#EDF3E7] via-[#E4ECD9] to-[#D5E2C4] px-4 py-8 md:px-10 lg:px-16 antialiased relative overflow-hidden">
      <div className="absolute top-10 left-10 h-64 w-64 rounded-full bg-[#6B8E23]/5 blur-[80px] pointer-events-none" />
      <div className="absolute bottom-10 right-10 h-64 w-64 rounded-full bg-[#2F3A2F]/5 blur-[80px] pointer-events-none" />

      <section className="relative mx-auto max-w-7xl z-10">
        {/* COMPACT HUB HEADER */}
        <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <span className="inline-flex items-center gap-1 rounded-full border border-[#6B8E23]/20 bg-white/40 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[#5A7A1E]">
              <span className="h-1 w-1 rounded-full bg-[#6B8E23] animate-pulse" /> FitSphere Command
            </span>
            <h1 className="text-3xl font-black tracking-tight text-[#2F3A2F] mt-1">Overview Terminal</h1>
          </div>
          <Link href="/dashboard/admin/classes" className="inline-flex h-10 items-center justify-center rounded-xl bg-gradient-to-r from-[#6B8E23] to-[#5A7A1E] px-5 text-xs font-bold text-white shadow-md shadow-[#6B8E23]/10 transition hover:opacity-95 active:scale-[0.98]">
            Review Classes
          </Link>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {topCards.map((c) => (
            <div key={c.label} className="group rounded-2xl border border-white/50 bg-white/35 p-5 shadow-md backdrop-blur-md transition hover:-translate-y-0.5 hover:bg-white/45">
              <div className="flex items-center justify-between text-[#5D6B57]">
              ...
              <h2 className="mt-2 text-2xl font-black text-[#2F3A2F] tracking-tight">{c.value}</h2>
              <p className="mt-1 text-[11px] font-medium text-[#6B785F]/80">{c.helper}</p>
            </div>
          ))}
        </div>
        ...
        {/* LOWER PLATFORM REPORT & RUNTIME CONTROLS */}
        <div className="mt-5 grid gap-5 lg:grid-cols-[1.3fr_1fr]">
          ...
          <div className="rounded-2xl border border-white/50 bg-white/35 p-5 shadow-md backdrop-blur-md flex flex-col justify-between gap-4">
            <div>
              <h2 className="text-lg font-black text-[#2F3A2F]">Quick Controls</h2>
            ...
            <div className="rounded-xl border border-white/40 bg-gradient-to-br from-[#DDE5D0]/60 to-[#C9D6B8]/60 p-4 shadow-inner flex items-center justify-between">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-wider text-[#5D6B57]">Pending Applications</p>
                <h3 className="text-2xl font-black text-[#2F3A2F] mt-0.5">{stats?.pendingTrainerApplications || 0}</h3>
              </div>
              <span className="text-[10px] font-bold text-[#6B8E23] bg-white/60 px-2 py-1 rounded-md shadow-sm">Action Needed</span>
            </div>
          </div>
        </div>
      </section>
...
function ChartCard({ title, children }) {
  return (
    <div className="rounded-2xl border border-white/50 bg-white/35 p-5 shadow-md backdrop-blur-md flex flex-col justify-between">
      <h2 className="text-sm font-black tracking-tight text-[#2F3A2F] mb-3">{title}</h2>
...
function LegendList({ data, colors }) {
  return (
    <div className="mt-2 grid grid-cols-3 gap-1.5">
      {data.map((item, i) => (
        <div key={item.name} className="flex flex-col items-center justify-center rounded-xl border-2 border-white/40 bg-white/20 py-1.5 text-center backdrop-blur-sm">
          <span className="flex items-center gap-1 text-[10px] font-bold text-[#5D6B57]">
            <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: colors[i % colors.length] }} />
            {item.name}
          </span>
          <span className="text-sm font-black text-[#2F3A2F] mt-0.5">{item.value}</span>
        </div>
      ))}
    </div>
  );
...
function QuickLink({ title, href }) {
  return (
    <Link href={href} className="flex h-10 items-center justify-center rounded-xl border border-white/30 bg-white/40 text-center text-xs font-bold text-[#2F3A2F] transition hover:-translate-y-0.5 hover:bg-white/60 active:translate-y-0 shadow-sm">
      {title}
    </Link>
  );
}
...
function CustomTooltip({ active, payload }) {
  if (active && payload && payload.length) {
    return (
      <div className="rounded-xl border border-white/60 bg-white/75 p-2.5 shadow-xl backdrop-blur-md text-xs font-bold text-[#2F3A2F]">
        <p className="opacity-80">{payload[0].name}</p>
        <p className="text-sm text-[#6B8E23] mt-0.5">{payload[0].value}</p>
      </div>
    );
  }
  return null;
